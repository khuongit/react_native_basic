...
export default class IOSList extends Component {
    constructor(props) {
        super(props);
        this.state = {
            so: 0,
    ...
    componentDidMount() {
        ...
        fetch(url)
            ...
            .then((responseData) => {
                let IOSList = responseData.filter((item)=>{ 
                    let ptt = new RegExp("^IPHONE\_[0-9]*");
                    return ptt.test(item.Ma_so); 
                });
                Du_lieu.IOSPhoneList = IOSList;
